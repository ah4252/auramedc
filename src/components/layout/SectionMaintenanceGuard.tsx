...
export default function SectionMaintenanceGuard({
  ...
  // جلب الإعدادات من API مباشرة عند كل تحميل لضمان أحدث حالة صيانة
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        // fallback to initialSettings already set
      }
    };
    fetchSettings();
  }, [pathname, searchParams]);

  // الكشف عن التبويب النشط
  const activeTab = searchParams.get("tab");
  const isAdminPath = pathname.startsWith("/admin");
  const isOwner = userEmail === "abendakfal07@gmail.com";

  // المدير وصاحب الحساب يتصفحان بشكل طبيعي دائماً
  if (isAdminPath || isAdminUser || isOwner) {
    return <>{children}</>;
  }

  // فحص وضع الصيانة لكل قسم
  ...
  if ((isPharmacyTab || isPharmacyPage) && settings?.maintenancePharmacy) {
    ...
    (pathname.startsWith("/qcms") ||
      (pathname.startsWith("/courses") && activeTab === "qcms")) &&
    settings?.maintenanceQcms
  ) {
    isUnderMaintenance = true;
