...
export async function createDiscussion(userId: string, title: string, content: string, subjectId?: string, categoryId?: string) {
  try {
    ...
    // Augment with social data safely
    const userData = await prisma.user.findUnique({
      where: { id: userId },
      select: { telegram: true, instagram: true, facebook: true }
    });

    if (userData) {
      newDiscussion.user.telegram = userData.telegram;
      newDiscussion.user.instagram = userData.instagram;
      newDiscussion.user.facebook = userData.facebook;
    }
