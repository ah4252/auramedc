...
export default function NewDiscussionModal({ isOpen, onClose, subjects, userId, categoryId, onSuccess, editPost }: { 
  isOpen: boolean, 
  onClose: () => void, 
  subjects: any[], 
  userId: string,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editPost) {
      ...
      const res = await createDiscussion(userId, title, content, subjectId || undefined, categoryId);
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 30 }} className="relative w-full max-w-lg bg-[#0a0f1d] border border-white/10 rounded-[3.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden">
            ...
            <div className="p-10 md:p-14 space-y-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
              <div className="flex items-center justify-between">
                ...
                <button onClick={onClose} className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-slate-500 hover:text-white hover:bg-red-500/20 transition-all" title="إغلاق">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative group">
                    ...
                    <select id="subject-select" value={subjectId} onChange={(e) => setSubjectId(e.target.value)} className="w-full p-5 pr-14 rounded-[1.5rem] border-2 border-white/5 bg-white/5 focus:border-medical-500 focus:bg-white/10 outline-none font-bold text-lg appearance-none transition-all text-white" aria-label="اختر المادة العلمية">
                ...
                <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl flex items-start gap-3">
                   ...
                   <p className="text-[11px] text-red-400 font-bold leading-tight">تنبيه: هذا المنشور مؤقت وسيتم حذفه تلقائياً من المجتمع بعد **24 ساعة** من الآن.</p>
                </div>
              ...
              <p className="text-[10px] text-center text-slate-600 font-bold">تذكر: سيتم حذف هذا المنشور تلقائياً بعد 24 ساعة من الآن.</p>
            </div>
          </motion.div>
        </div>
      )}
