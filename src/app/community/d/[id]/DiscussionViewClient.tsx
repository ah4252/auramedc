...
import { addComment, deleteDiscussionComment, toggleLike, updateDiscussion, deleteDiscussion } from "../../../actions/community";
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {discussion.user?.telegram && (
                      <a href={discussion.user.telegram.startsWith('http') ? discussion.user.telegram : `https://t.me/${discussion.user.telegram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="p-1.5 bg-sky-500/10 text-sky-500 rounded-lg hover:bg-sky-500 hover:text-white transition-all shadow-sm">
                        <Send className="w-3.5 h-3.5" />
                      </a>
                    )}
                    {discussion.user?.instagram && (
                      <a href={discussion.user.instagram.startsWith('http') ? discussion.user.instagram : `https://instagram.com/${discussion.user.instagram}`} target="_blank" rel="noopener noreferrer" className="p-1.5 bg-pink-500/10 text-pink-500 rounded-lg hover:bg-pink-500 hover:text-white transition-all shadow-sm">
                        <Instagram className="w-3.5 h-3.5" />
                      </a>
                    )}
                    {discussion.user?.facebook && (
                      <a href={discussion.user.facebook.startsWith('http') ? discussion.user.facebook : `https://facebook.com/${discussion.user.facebook}`} target="_blank" rel="noopener noreferrer" className="p-1.5 bg-blue-600/10 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm">
            ...
            {user?.id === discussion.userId && !isEditing && (
              <div className="flex items-center gap-2">
                 <button onClick={() => setIsEditing(true)} className="p-3 bg-white/5 hover:bg-blue-500/20 text-slate-400 hover:text-blue-400 rounded-2xl transition-all">
                    <Edit3 className="w-5 h-5" />
                 </button>
                 <button onClick={handleDeletePost} className="p-3 bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-400 rounded-2xl transition-all">
          ...
          <AnimatePresence mode="wait">
            {isEditing ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                 <input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="w-full bg-black/40 border-2 border-medical-500/30 p-5 rounded-2xl outline-none font-black text-2xl text-white" />
                 <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} rows={6} className="w-full bg-black/40 border-2 border-medical-500/30 p-5 rounded-2xl outline-none font-bold text-lg text-slate-300 resize-none" />
                 <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-start justify-between mb-4">
                           ...
                           <div className="flex items-center gap-3">
                              ...
                              <div className="flex items-center gap-1.5">
                                {comment.user?.telegram && (
                                  <a href={comment.user.telegram.startsWith('http') ? comment.user.telegram : `https://t.me/${comment.user.telegram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="text-sky-500 hover:text-sky-400">
                                    <Send className="w-3 h-3" />
                                  </a>
                                )}
                                {comment.user?.instagram && (
                                  <a href={comment.user.instagram.startsWith('http') ? comment.user.instagram : `https://instagram.com/${comment.user.instagram}`} target="_blank" rel="noopener noreferrer" className="text-pink-500 hover:text-pink-400">
                                    <Instagram className="w-3 h-3" />
                                  </a>
                                )}
                                {comment.user?.facebook && (
                                  <a href={comment.user.facebook.startsWith('http') ? comment.user.facebook : `https://facebook.com/${comment.user.facebook}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-500">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {(user?.id === comment.userId || user?.role === "ADMIN") && (
                          <button onClick={() => handleDeleteComment(comment.id)} className="p-3 text-slate-600 hover:text-red-400 hover:bg-red-500/10 rounded-2xl transition-all"><Trash2 className="w-5 h-5" /></button>
                        )}
                      </div>
                    </div>
                      ...
                      {replyToId === comment.id && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-6 pt-6 border-t border-white/5">
                           <form onSubmit={(e) => handleAddComment(e, comment.id)} className="flex gap-4">
                              ...
                              <button type="submit" disabled={!replyContent || loading} className="p-4 bg-medical-600 text-white rounded-2xl hover:bg-medical-500 transition-all"><Send className="w-5 h-5 -rotate-45" /></button>
                           </form>
                        </motion.div>
                      )}
                    </AnimatePresence>
                        ...
                        >
                          <div className="flex items-start justify-between mb-2">
                            ...
                            {(user?.id === reply.userId || user?.role === "ADMIN") && (
                              <button onClick={() => handleDeleteComment(reply.id)} className="p-2 text-slate-700 hover:text-red-400 transition-all"><Trash2 className="w-4 h-4" /></button>
                            )}
                          </div>
