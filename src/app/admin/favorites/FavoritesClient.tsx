                ...
                <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                  <div className="w-14 h-14 rounded-full border-2 border-white dark:border-slate-700 shadow-sm overflow-hidden">
                    {selectedFav.user.image ? (
                      <img src={selectedFav.user.image} alt={selectedFav.user.name || "صورة المستخدم"} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 group shadow-sm bg-slate-50 dark:bg-slate-800/30">
                    <div className="aspect-video relative bg-[#05070a]">
                      {getYoutubeThumbnail(selectedFav.lesson.videoUrl) ? (
                        <>
                          <img 
                            src={getYoutubeThumbnail(selectedFav.lesson.videoUrl) || undefined} 
                            alt={selectedFav.lesson.title || "معاينة الدرس"}
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900 text-white gap-3 border-b border-white/5">
                           <div className="w-14 h-14 bg-white/5 rounded-full flex items-center justify-center border border-white/10 shadow-inner">
                              <BookOpen className="w-7 h-7 text-white opacity-60" />
                           </div>
                           <span className="text-lg font-black uppercase tracking-widest text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]">لا يوجد فيديو</span>
                        </div>
                      )}
                    </div>
                    <div className="p-4">
                      <a 
                        href={`/courses/v/${selectedFav.lesson.slug}`}
                        target="_blank"
