...
import { 
  ...
  Lock, ShieldCheck, KeyRound, Zap, GraduationCap, FlaskConical, NotebookPen, Sparkles
} from "lucide-react";
...
export default function AdminSidebar({ 
  ...
  // Links that are NOT protected (exempt from tools password)
  const exemptPaths = ["/admin/news"];

  const isProtectedPath = (href: string) => {
    if (!toolsProtected || unlocked) return false;
    return !exemptPaths.some(exempt => href === exempt);
  };

  const handleLockTools = async () => {
    await lockTools();
    setUnlocked(false);
    router.refresh();
  };
    ...
    <>
      ...
      `}>
        ...
        <nav className="flex-1 p-6 space-y-8 overflow-y-auto custom-scrollbar">
            ...
            <div key={idx} className="space-y-3">
              ...
              <div className="space-y-1">
                {group.links.map((link) => {
                  ...
                  const isLocked = isProtectedPath(link.href);
          ...
          {/* Lock/Unlock Tools Button */}
          {toolsProtected && unlocked && (
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
              <button
                onClick={handleLockTools}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 rounded-xl text-xs font-black hover:bg-amber-500 hover:text-white transition-all"
              >
                <Lock className="w-3.5 h-3.5" />
                <span>قفل الأدوات الآن</span>
              </button>
            </div>
          )}
        </nav>
